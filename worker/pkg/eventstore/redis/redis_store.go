...
func (r *RedisEventStore) subscriptionLoop(ctx context.Context, config eventstore.SubscriptionConfig, eventCh chan<- *cloudevents.CloudEvent, errorCh chan<- error) {
	...
	r.logger.Info("DEBUG: Starting subscription loop", "streams", config.Streams, "consumer_group", config.ConsumerGroup)

	for {
		select {
		case <-ctx.Done():
			r.logger.Info("DEBUG: Context cancelled, stopping subscription loop")
			return
		default:
			// Read from streams
			...
			r.logger.Info("DEBUG: Reading from Redis streams", "streams", streams, "consumer_group", config.ConsumerGroup, "consumer_name", config.ConsumerName)
			...
			r.logger.Info("DEBUG: XReadGroup result", "error", err, "result_count", len(result))
			...
			for _, stream := range result {
				r.logger.Info("DEBUG: Processing stream", "stream_name", stream.Stream, "message_count", len(stream.Messages))
				for _, message := range stream.Messages {
					r.logger.Info("DEBUG: Processing message", "message_id", message.ID, "values", message.Values)
					event, err := r.parseMessage(message)
					if err != nil {
						r.logger.Error("DEBUG: Failed to parse message", "message_id", message.ID, "error", err)
						select {
					...
					r.logger.Info("DEBUG: Successfully parsed event", "event_id", event.ID, "event_type", event.Type)

					// Apply filters
					shouldProcess := r.shouldProcessEvent(event, config)
					r.logger.Info("DEBUG: Event filtering",
						"event_id", event.ID,
						"event_type", event.Type,
						"should_process", shouldProcess,
						"config_event_types", config.EventTypes)

					if shouldProcess {
						r.logger.Info("DEBUG: Sending event to channel", "event_id", event.ID)
						select {
						case eventCh <- event:
							r.logger.Info("DEBUG: Event sent to channel successfully", "event_id", event.ID)
							// Acknowledge message
						...
						r.logger.Info("DEBUG: Event filtered out", "event_id", event.ID, "event_type", event.Type)
